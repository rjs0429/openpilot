...
from openpilot.mdpilot.features.forward_watch.watch import (
...
class TestForwardWatch(unittest.TestCase):
  def test_toggle_is_the_enable_gate(self):
  ...
  def test_park_cancels_and_drive_rearms(self):
  ...
  def test_timeout_wins_over_departure_trigger(self):
    fw = ForwardWatch(FakeParams())
    sm = build_sm()
    advance_to_monitoring(fw, sm)
  ...
  def test_timeout_wins_over_signal_change(self):
    fw = ForwardWatch(FakeParams())
    sm = build_sm()
    advance_to_monitoring(fw, sm)
  ...
  def test_timeout_cancels_active_alert(self):
    fw = ForwardWatch(FakeParams())
    sm = build_sm()
  ...
  def test_timeout_does_not_rearm_until_stopped_session_ends(self):
    fw = ForwardWatch(FakeParams())
    sm = build_sm()
    advance_to_monitoring(fw, sm)
