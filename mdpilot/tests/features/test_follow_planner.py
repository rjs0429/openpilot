...
class TestRequiredDecel(unittest.TestCase):
...
class TestFollowPlanner(unittest.TestCase):
  @staticmethod
...
class TestWithPortCarParams(unittest.TestCase):
  """Runs the planner on the car port's own CarParams rather than a hand-filled one."""
  ...
  def test_free_road_follows_the_set_speed(self):
    ...
    follower = Follower()
    follower.planner = LongitudinalPlanner(CP)
    follower.follow = FollowPlanner(CP)
